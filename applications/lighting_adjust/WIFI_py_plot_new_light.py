...
lines2, = ax2.plot([], [], label='IMU_roll',color='PaleVioletRed')
...
def use_data(timestamp, data_values):  
    ...
    signal=light_control.pre_process(data_values)
    bright_command=light_control.get_bright_command(signal)
    
    color_command=light_control.get_color_command(data_values)
    #print(command)
    
    if (bright_command!=b'8'):
        ser.write(bright_command) #写入命令
...
def update_plot(frame):
    ...
    lines2.set_data([timestamp for timestamp, _ in data_buffer], [data_values[8] for _, data_values in data_buffer]) #时间戳作为横坐标
