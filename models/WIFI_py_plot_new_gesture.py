...
engine.setProperty('rate', 200) #一般范围一般在0~500之间

voices = engine.getProperty('voices')  
engine.setProperty('voice', voices[1].id)#男性（voices[0].id）、女性（voices[1].id）

volume = engine.getProperty('volume')                         
engine.setProperty('volume',1) #调整声量范围在0~1之间

#ClassSet=['B', 'Bye', 'C', 'Good', 'Hello', 'I', 'J', 'One', 'Yes', 'Z']
def use_data(timestamp, data_values):  
    #print(timestamp, data_values)
    result, probabilities=gesture_recognition.recognition(data_values)
    if result!=None:
        print("预测结果:", result)
        gesture_recognition.show_result(result,window)
        if result!='wait':
            engine.say(result)
            engine.runAndWait() 
        #print("test")
...
if __name__ == "__main__":
    ...
    root2.withdraw() 
    root2 = tk.Toplevel()
    window = gesture_recognition.ResultWindow(root2)
